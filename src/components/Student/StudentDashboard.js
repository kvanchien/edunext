...
const StudentDashboard = () => {
  ...
  const renderUserInfo = () => {
    if (!loggedInStudent) return  <p>Loading...</p>;

    const userCourses = courses.filter(course => loggedInStudent.courses.includes(course.id));
    
    return (
      <div>
        <p>
          <b>Name:</b> {loggedInStudent.name}
        </p>
        <p>
          <b>Campus:</b> {campus}
        </p>
        ...
        <p>
          <b>Class:</b> {loggedInStudent.class}
        </p>
        <p>
          <b>Courses:</b>
        </p>
        <ul>
          {userCourses.map((course) => (
            <li key={course.id}>
              {course.name} (Teacher: {getTeacherName(course.teacher)})
            </li>
          ))}
        </ul>
      </div>
